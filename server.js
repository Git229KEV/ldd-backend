  ...
  file.download()
    .then(data => {
      ...
      fs.writeFile(localFilePath, fileBuffer, (err) => {
        ...
        res.status(200).send(`File '${filename}' fetched and saved successfully!`);
      });
    })
    .catch(err => {
      console.error(err);
      res.status(500).send('Error fetching file from Firebase Storage.');
    });
});
...
app.post("/api/verify-document", async (req, res) => {
  const { filename, docType, ...userData } = req.body;
  ...
  try {
    ...
    console.log(`File downloaded from Firebase: ${filename}`);

    // Convert to base64
    const base64Pdf = fs.readFileSync(localFilePath, "base64");

    // Convert PDF → images
    const images = await convertPDFtoImages(localFilePath, tempDir);
    if (images.length === 0) {
    ...
    const comparisonResult = await extractAndCompare(base64Pdf, userData, docType);
